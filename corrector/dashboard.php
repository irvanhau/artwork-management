...
if ($proses_id > 0 && $specialist_id > 0) {
    $where = "WHERE ($proses_id = proses.process_id AND $specialist_id = job_order.specialist_id) and job_order.corrector_id=$id";
} elseif ($proses_id > 0) {
    $where = "WHERE $proses_id = proses.process_id and job_order.corrector_id=$id";
} else if ($specialist_id > 0) {
    $where = "WHERE $specialist_id = job_order.specialist_id and job_order.corrector_id=$id";
} else {
    $where = "WHERE job_order.corrector_id=$id";
}
$likeKeyword = $keyword != "" ? "AND job_order.prod_name LIKE '%$keyword%'" : "";
?>
        ...
        <div class="content">
            ...
            <div class="col-md-12 py-3 px-3">
                ...
                <table class="table table-bordered">
                    <?php
                    $queryJob = mysqli_query($koneksi, "SELECT product.product_name as prodName, job_order.*,specialist.username as specialist,drafter.username as drafter,proses.process_id 
                    from job_order 
                    join specialist on job_order.specialist_id=specialist.id 
                    join proses on proses.job_id=job_order.jo_number 
                    join drafter on drafter.id=job_order.drafter_id 
                    ...
                    if (mysqli_num_rows($queryJob) == 0) {
                        ...
                        while ($row = mysqli_fetch_assoc($queryJob)) {
                            ...
                            echo "<tbody class='isi'>
                                    ...
                                    <td>$proses</td>
                                    <td>
                                        <a href='" . BASE_URL3 . "detail.php?job_id=$row[jo_number]'><button class='btn btn-danger' type='button'>Detail</button></a>
                                    </td>
                                </tbody>";
                            $no++;
                        }
