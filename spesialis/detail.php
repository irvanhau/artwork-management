...
$queryJob = mysqli_query($koneksi, "SELECT job_order.*,specialist.username,proses.process_id from job_order 
join specialist on job_order.specialist_id=specialist.id join proses on proses.job_id=job_order.jo_number 
where job_order.jo_number = '$job_id'");

$queryDraft = mysqli_query($koneksi, "SELECT artwork_draft.* from artwork_draft join job_order 
on artwork_draft.job_id=job_order.jo_number where artwork_draft.job_id=$job_id");

$queryFinal = mysqli_query($koneksi, "SELECT artwork_final.* from artwork_final join job_order
 on artwork_final.job_id=job_order.jo_number where artwork_final.job_id=$job_id");

$queryProses = mysqli_query($koneksi, "SELECT proses.* from proses join job_order 
on proses.job_id=job_order.jo_number where proses.job_id=$job_id");
            ...
            <div class="col-md-12 py-3 px-3">
                ...
                <div class="row mb-3">
                    <div class="col">
                        ...
                        <select onchange="cek_database()" disabled class="form-select mb-3" name="product_id" id="product_name" aria-label=".form-select-lg example">
                ...
                <div class="row">
                    <div class="col">
                        ...
                        <input type="text" class="form-control" value="<?php echo $specialist_name; ?>" id="material" aria-label="material" disabled>
                    </div>
                    <div class="col">
                ...
                <div class=" col">
                    <?php
                    if ($rowProses['process_id'] == 2 || $rowProses['process_id'] == 3) {
                        echo "<label for='ArtwrokDraft' class='form-label'>Artwrok Draft</label>";
                        echo "<input type='text' class='form-control mb-2' name='artwork_draft' value='$rowDraft[artwork_draft]' disabled>";
                        echo "<a href='" . BASE_URL . "download.php?file=$rowDraft[artwork_draft]&name=artwork-draft'>
                        <button type='submit' class='btn btn-primary'>Download Artwork Draft</button></a>";
                    } else {
                        echo "
                        <label for='ArtwrokDraft' class='form-label d-none'>Artwrok Draft</label>
                                <input type='file' class='form-control mb-2 d-none' disabled name='artwork_draft'>
                                <button type='submit' class='btn btn-primary d-none' disabled>Download Artwork Draft</button>
                                ";
                    }
                    ?>
                </div>
                <div class="col">
                    ...
                    if ($rowProses['process_id'] == 3) {
                        ...
                        echo "<a href='" . BASE_URL
                            . "download.php?file=$rowFinal[artwork_final]&name=artwork-final'>
                        <button type='submit' class='btn btn-primary'>Download Artwrok Final</button></a>";
                    } else {
                        echo "
                                <label for='ArtwrokFinal' class='form-label d-none'>Artwrok Final</label>
                                <input type='file' class='form-control mb-2 d-none' disabled name='artwork_final'>
